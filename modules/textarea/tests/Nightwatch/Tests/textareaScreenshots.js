/**
 * @file
 * Captures textarea testform with different states.
 *
...
module.exports = {
  ...
  "Textarea form": function test(browser) {
    ["", "he"].forEach(langprefix => {
      browser
        .resizeWindow(1024, 600)
        .smartURL(
          langprefix ? `/${langprefix}/contact/textarea` : "/contact/textarea"
        )
        .setValueAndChange(
          '[name="message[0][value]"]',
          "Test message body with error"
        )
        .waitTillElementPresent(
          '[name="formatted[0][value]"] + .cke .cke_wysiwyg_frame',
          5000
        )
        ...
        .click(
          '[class*="js-form-item-"][class*="-formatted-summary-0-value"] .link-edit-summary'
        )
        .waitForElementVisible('[name="formatted_summary[0][summary]"]', 5000)
        .setValueAndChange(
          '[name="formatted_summary[0][summary]"]',
          "..some summary here, but no text in main area!"
        )
        .pause(300)
        ...
        .elements(
          ...
          inlineFormMessageQueryResults => {
            browser.savefullScreenShot(
              inlineFormMessageQueryResults.value.length ? "03" : "02",
              langprefix,
              inlineFormMessageQueryResults.value.length
                ? "Inline errors"
                : "Errors"
            );
          }
